...
const Profile:Component =()=> {
  ...
  const fetchUser = async () => {
      ...
      const res = await fetch("https://13.49.228.160443/api/users/me", {
        method: "GET",
        mode: "cors",
        credentials: "include",
      });
  ...
  createEffect(() => {
    if (getUser()) {
      ...
      instance.get(`https://13.49.228.160443/api/users/${theUsr.id}/posts`).then((thePosts) => {
      ...
      instance.get(`https://13.49.228.160443/api/users/${user.id}/followers`).then((res)=> {
        setMyFollowers(res.data)
        console.log(res.data,"done deal")
     
      })

      instance.get(`https://13.49.228.160443/api/users/${user.id}/following`).then((res)=> {
