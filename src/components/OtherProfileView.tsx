...
const OtherProfileView = (props:MyProfileViewProps)=> {
    ...
    createEffect(()=>{
        instance.get(`http://localhost:8000/api/users/${props.followingID}/following`).then((res)=> {
            console.log("this is", res.data)
            setOtherFollowing(res.data)
    
          })

          instance.get(`http://localhost:8000/api/users/${props.followingID}/followers`).then((res)=> {
            console.log("this is", res.data)
            setOtherFollowers(res.data)
    
          })

          instance.get(`http://localhost:8000/api/users/${props.followerID}/following`).then((res)=> {
            console.log("this is", res.data)
            setOtherFollowing(res.data)
    ...
    return (
        ...
        <Show when={isFollowing()} fallback={
              <button class="bg-white rounded-[4px]  w-[8rem] text-center h-[2.5rem]" onClick={()=> {
                ...
                instance.post(`http://localhost:8000/api/users/263945d0-453c/following`, {
            ...
            <button class="bg-white rounded-[4px]  w-[10rem] text-center h-[2.5rem]" onClick={()=> {
                ...
                instance.post(`http://localhost:8000/api/users/263945d0-453c/unfollow`, {
                    followerID: props.followerID,
                    followingID: props.followingID
                }).then((response)=>{
                    console.log(response.data)
